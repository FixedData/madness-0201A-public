...
class Character extends FlxSprite
{
	...
	**/
	...
	public var preAnimationCallback:AnimationCallData -> Void;
	...
	public function new(x:Float, y:Float, ?character:String = 'bf', ?isPlayer:Bool = false)
	{
		...
		{
			case 'creditChar':
				final path = Paths.getSharedPath('images/madnessmenu/credits/' + 'creditChar' + '.json');
...
public function playAnim(AnimName:String, Force:Bool = false, Reversed:Bool = false, Frame:Int = 0, ?overwriteAnim:Bool = false):Void
	{
		//might be useful for some other shit beside bendy idk
		var animCallData:AnimationCallData = {
		AnimName:AnimName,
		Force:Force,
		Reversed:Reversed,
		Frame:Frame,
		overwriteAnim: overwriteAnim,
		overrideCall: false
		};


		if (preAnimationCallback != null && animation.curAnim != null)
		preAnimationCallback(animCallData);

		if (animCallData.overrideCall) return;

		specialAnim = false;
		if(!isAnimateAtlas) animation.play(animCallData.AnimName, animCallData.Force, animCallData.Reversed, animCallData.Frame);
		else atlas.anim.play(animCallData.AnimName, animCallData.Force, animCallData.Reversed, animCallData.Frame);

		if (animOffsets.exists(animCallData.AnimName))
		{
			var daOffset = animOffsets.get(animCallData.AnimName);
		...
		{
			if (animCallData.AnimName == 'singLEFT')
				danced = true;

			else if (animCallData.AnimName == 'singRIGHT')
				danced = false;

			if (animCallData.AnimName == 'singUP' || animCallData.AnimName == 'singDOWN')
				danced = !danced;
		}
	}
...
typedef AnimationCallData = 
{
	var AnimName:String; 
	var Force:Bool;
	var Reversed:Bool; 
	var Frame:Int;
	var ?overwriteAnim:Bool;
	var overrideCall:Bool;
